...
import { supabaseChannelOps } from "@/lib/supabase-direct"
import { successResponse, errorResponse } from "@/lib/api-response"

// 获取所有频道
export async function GET() {
  try {
    const channels = await supabaseChannelOps.getChannels()
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'add') {
      try {
        const newChannel = await supabaseChannelOps.addChannel(channelData)
      ...
      const updatedChannel = await supabaseChannelOps.updateChannel(channelId, channelData)
      ...
      const success = await supabaseChannelOps.deleteChannel(channelId)
